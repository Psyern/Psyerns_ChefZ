...
```
"<AB>" "<repo>\ChefZ_Core\Addons\ChefZ_Core" "<out>" -clear -packonly -prefix=ChefZ_Core
```

**Variante B — wie später im Release (config.cpp wird zu config.bin):**

```
"<AB>" "<repo>\ChefZ_Core\Addons\ChefZ_Core" "<out>" -clear -prefix=ChefZ_Core -include=<abs>\include.txt
```
