...
class ChefZ_ProcessCompiler
{
    ...
    ChefZ_CompiledProcess CompileProcess(ChefZ_ProcessDef def, ChefZ_ToolRegistry tools)
    {
        ...
        if (exec < 0)
        {
            ...
            Fail(def, "unbekannte Ausfuehrungsform \"" + def.exec + "\" - Prozess abgewiesen. " + "Gueltig: " + ChefZ_ProcessExec.ValidNames() + ".");
            return null;
        }
        ...
        if (proc.hasMinTemperature && proc.hasMaxTemperature && proc.minTemperature > proc.maxTemperature)
        {
            ...
            Warn(def, "minTemperature (" + proc.minTemperature.ToString() + ") ist groesser " + "als maxTemperature (" + proc.maxTemperature.ToString() + ") - die Grenzen werden getauscht.");
    ...
    private bool CompileToolGroups(notnull ChefZ_ProcessDef def, notnull ChefZ_CompiledProcess proc, ChefZ_ToolRegistry tools)
    {
        ...
        for (int i = 0; i < declared; i++)
        {
            ...
            {
                Fail(def, "toolGroups[" + i.ToString() + "] nennt die Werkzeuggruppe \"" + name + "\", die in keinem CfgChefZTools-Eintrag vorkommt - Prozess " + "ABGEWIESEN (11 §7). Nicht ignoriert: ohne Werkzeugpruefung waere der " + "Prozess ohne Werkzeug ausloesbar, und das faellt niemandem auf.");
                return false;
            }
     ...
     */
    private float ResolveDuration(notnull ChefZ_ProcessDef def, notnull ChefZ_CompiledProcess proc)
    {
        ...
        Warn(def, "baseDurationSec = " + d.ToString() + " ist fuer einen " + ChefZ_ProcessExec.Name(proc.exec) + "-Prozess zu klein und wird auf " + ChefZ_ProcessingLimits.MIN_DURATION_SEC.ToString() + " geklemmt. Ein Stationsjob " + "ohne Dauer waere von \"sofort\" nicht zu unterscheiden, und der " + "Fortschrittsbalken waere nur Flackern.");
    ...
    private int ClampToolDamage(notnull ChefZ_ProcessDef def)
    {
        ...
        if (d < 0)
        {
            Warn(def, "toolDamage = " + d.ToString() + " ist negativ und wird auf 0 geklemmt. " + "Ein Prozess, der Werkzeuge REPARIERT, ist keine Funktion dieses Systems.");
            return 0;
        }
        if (d > 100)
        {
            Warn(def, "toolDamage = " + d.ToString() + " liegt ueber 100 und wird geklemmt. " + "100 ist der volle Gesundheitsbereich eines Items.");
            return 100;
        }
        return d;
    }
     ...
     */
    private void WarnAboutMisplacedFields(notnull ChefZ_ProcessDef def, notnull ChefZ_CompiledProcess proc)
    {
        ...
        {
            Warn(def, "animationLength und specialty wirken nur bei HANDCRAFT (11 §2) - " + "dieser Prozess ist " + ChefZ_ProcessExec.Name(proc.exec) + " und ignoriert sie. Die Dauer eines Stationsjobs steht in " + "baseDurationSec.");
        }
    }
    ...
    ChefZ_CompiledStation CompileStation(ChefZ_StationDef def, notnull map<int, int> known)
    {
        ...
        for (int i = 0; i < declared; i++)
        {
            ...
            {
                Fail(def, "processes[" + i.ToString() + "] nennt den Prozess \"" + name + "\", den es nicht gibt - der EINTRAG wird verworfen. Die uebrigen " + "Prozesse dieser Station funktionieren weiter (11 §7). Fehlt das " + "Content-Modul, oder ist der Name falsch geschrieben?");
                continue;
            }
            ...
            if (station.processes.Find(sym) >= 0)
            {
                ...
                Warn(def, "Prozess \"" + name + "\" steht mehrfach in processes[] - der " + "zweite Eintrag wird ausgelassen.");
                continue;
            }
        ...
        {
            Warn(def, "Die Station bietet " + station.processes.Count().ToString() + " Prozesse an; synchronisierbar sind " + (ChefZ_ProcessingLimits.PROCESS_ORDINAL_MAX + 1).ToString() + ". Die uebrigen sind serverseitig nutzbar, aber der Client kann den " + "aktiven Prozess dann nicht anzeigen.");
        }

        if (declared == 0)
        {
            Warn(def, "Die Station nennt keinen einzigen Prozess - sie ist inerte Deko. " + "Das ist zulaessig; falls es ein Versehen war, fehlt processes[].");
        }
    ...
    private void VerifyStationClass(notnull ChefZ_StationDef def)
    {
        ...
        Warn(def, "Zu diesem CfgChefZStations-Eintrag gibt es keine CfgVehicles-Klasse \"" + def.id + "\". Der Eintrag bleibt geladen und ist wirkungslos, bis die Klasse " + "da ist (11 §2: die id IST der Klassenname).");
    }
     ...
     */
    ChefZ_CompiledTransform CompileTransform(ChefZ_TransformDef def, notnull map<int, ref ChefZ_CompiledProcess> processes, notnull map<int, int> stations)
    {
        if (!def)
            return null;
        ...
        if (!m_Ctx)
        {
            Fail(def, "kein Selektorkontext vorhanden - der Config Manager hat die Registries " + "noch nicht gebaut. Der Transform wird uebersprungen.");
            return null;
        }
        ...
        {
            Fail(def, "nennt den Prozess \"" + def.process + "\", den es nicht gibt - " + "Transform abgewiesen (11 §7). Kein Prozess heisst: keine Station koennte " + "ihn anbieten und keine Aktion ihn ausloesen.");
            return null;
        }
    ...
    private bool CompileInputs(notnull ChefZ_TransformDef def, notnull ChefZ_CompiledTransform tr, notnull ChefZ_CompiledProcess proc)
    {
        ...
        {
            Fail(def, "Transform abgewiesen, weil ein Eingang nicht uebersetzt werden konnte: " + error + ". Ein Transform mit weggefallenem Pflichteingang waere viel zu " + "leicht ausloesbar (07 §7).");
            return false;
        }
        ...
        {
            Fail(def, "Transform hat keinen einzigen Pflichteingang - alle " + tr.inputs.Count().ToString() + " Eingaenge sind optional oder verlangen 0 " + "Items. Er wuerde an jeder Station zuenden, auch an einer leeren.");
            return false;
        }
         ...
         * niemand findet.
         */
        if (proc.exec == ChefZ_ProcessExec.HANDCRAFT && tr.inputs.Count() > ChefZ_ProcessingLimits.HANDCRAFT_MAX_INPUTS)
        {
            Fail(def, "Der Prozess \"" + proc.id + "\" ist HANDCRAFT und laeuft damit ueber " + "Vanillas Craftsystem; das kennt hoechstens " + ChefZ_ProcessingLimits.HANDCRAFT_MAX_INPUTS.ToString() + " Zutaten (01 V12). " + "Dieser Transform hat " + tr.inputs.Count().ToString() + " - abgewiesen. Abhilfe: den Prozess auf STATION_ACTION oder STATION_TIMED " + "umstellen; Stationen haben diese Grenze nicht.");
            return false;
        }
    ...
    private bool CompileOutputs(notnull ChefZ_TransformDef def, notnull ChefZ_CompiledTransform tr)
    {
        tr.outputs.Clear();
        tr.byproducts.Clear();
        ...
        {
            Fail(def, "Transform ohne \"outputs\" - abgewiesen. Er wuerde die Eingaenge " + "verbrauchen und nichts erzeugen.");
            return false;
        }
        ...
        for (int i = 0; i < def.outputs.Count(); i++)
        {
            ...
            if (o.cls == "")
            {
                ...
                if (o.setState == "")
                {
                    Fail(def, where + " nennt weder \"cls\" noch \"setState\" - abgewiesen. " + "Ein Ergebnis muss entweder eine Klasse erzeugen oder einen Zustand " + "setzen (11 §2).");
                    return false;
                }
            ...
            if (o.variants)
            {
                for (int v = 0; v < o.variants.Count(); v++)
                {
                    ...
                    if (!variant || variant.cls == "")
                        continue;
                    if (!CheckOutputClass(def, variant.cls, where + ".variants[" + v.ToString() + "]"))
                        return false;
                }
            }
        ...
        {
            Fail(def, "Transform hat nach dem Pruefen kein einziges brauchbares Ergebnis mehr " + "- abgewiesen.");
            return false;
        }
        ...
        {
            Fail(def, "outputs[] mischt Klassentausch (" + withClass.ToString() + " mit \"cls\") und reinen Zustandswechsel (" + withoutClass.ToString() + " ohne \"cls\") - abgewiesen. Das eine verbraucht die Eingaenge, das andere " + "laesst sie bestehen; beides zugleich ist nicht bestimmbar. Trenne es in " + "zwei Transforms.");
            return false;
        }
        ...
        if (def.byproducts)
        {
            for (int b = 0; b < def.byproducts.Count(); b++)
            {
                ...
                if (by.cls == "")
                {
                    Fail(def, byWhere + " hat kein \"cls\" - abgewiesen. Ein Nebenprodukt " + "entsteht NEU; ein reiner Zustandswechsel hat dort kein Ziel.");
                    return false;
                }
        ...
        {
            Fail(def, "Ein reiner Zustandswechsel kann keine Nebenprodukte erzeugen - " + "abgewiesen. Er verbraucht nichts und erzeugt nichts; er setzt nur den " + "Zustand der Eingaenge (11 §2).");
            return false;
        }
     ...
     */
    private bool CheckPortionFields(notnull ChefZ_TransformDef def, notnull ChefZ_OutputDef o, string where)
    {
    ...
    private bool CheckOutputClass(notnull ChefZ_TransformDef def, string cls, string where)
    {
        ...
        {
            Fail(def, where + " nennt die Klasse \"" + cls + "\", die es in CfgVehicles nicht " + "gibt - Transform abgewiesen. Fehlt das Content-Modul, oder ist der Name " + "falsch geschrieben?");
            return false;
        }
        ...
        Fail(def, where + ": die essbare Klasse \"" + cls + "\" " + reason + " - Transform abgewiesen. PlayerStomach.InitData registriert sie damit nicht, " + "und AddToStomach bricht beim Verzehr OHNE MELDUNG ab: das Ergebnis verschwaende, " + "ohne zu saettigen (01 V7).");
        return false;
    }
     ...
     */
    private bool CheckStateChangeConsistency(notnull ChefZ_TransformDef def, notnull ChefZ_CompiledTransform tr)
    {
        ...
        for (int i = 0; i < tr.inputs.Count(); i++)
        {
            ...
            Fail(def, "Eingang \"" + slot.slotId + "\" hat consume \"" + ChefZ_ConsumeMode.Name(slot.consumeMode) + "\", der Transform ist aber ein " + "reiner Zustandswechsel (kein Ergebnis nennt eine Klasse) - abgewiesen. Er " + "wuerde die Zutat verbrauchen und nichts erzeugen. Setze \"consume\": \"" + ChefZ_ConsumeMode.NONE_NAME + "\" an allen Eingaengen.");
            return false;
        }
     ...
     */
    private void CompileStations(notnull ChefZ_TransformDef def, notnull ChefZ_CompiledTransform tr, notnull map<int, int> stations)
    {
        ...
        for (int i = 0; i < declared; i++)
        {
            ...
            {
                Warn(def, "stationsAllowed[" + i.ToString() + "] nennt die Station \"" + name + "\", die dieser Server nicht kennt. Der Transform bleibt geladen und der " + "Eintrag steht weiter in der Liste - die Station kann aus einem " + "optionalen Modul kommen (11 §7).");
            }
        }
    }

    private void CompileTuning(notnull ChefZ_TransformDef def, notnull ChefZ_CompiledTransform tr, notnull ChefZ_CompiledProcess proc)
    {
        if (def.HasDurationOverride())
        {
            ...
            if (d < 0.0)
            {
                Warn(def, "durationOverrideSec ist negativ (" + d.ToString() + ") und wird ignoriert - es gilt die Prozessdauer von " + proc.baseDurationSec.ToString() + "s.");
            }
            else
            {
        ...
        {
            Warn(def, "freshnessCarry = " + tr.freshnessCarry.ToString() + " liegt ausserhalb von [0, 1] und wird geklemmt. Ein Wert ueber 1 hiesse, " + "dass Verarbeiten eine Zutat FRISCHER macht - das waere ein Waschgang fuer " + "altes Fleisch (12 §4.1).");
    ...
    private void CompileRequirements(notnull ChefZ_TransformDef def, notnull ChefZ_CompiledTransform tr)
    {
        tr.requires.Clear();
        if (!def.requires)
            return;
