...
| Ingredients with no in-game source | `Rice` and `Honey` (required by Milk Rice and Honey Bread Platter) are vanilla loot. `ChefZ_Egg` is a loot-only ChefZ class (milk is vanilla `PowderedMilk`), and since 2026-08-29 so are wheat, the four vegetables and the six herbs (found like mushrooms) — and the mod packs **no `types.xml`**, so nothing spawns without an admin — except corn, thyme, rosemary and parsley, which since 2026-08-31 also grow wild: the CE templates in `ChefZ_Farming/ServerConfig/`, once installed into the mission, spawn harvest points that yield them. See [Known-Limitations](Known-Limitations). |
